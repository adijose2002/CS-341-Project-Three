...
// More details?
//
// Adithya Jose, UIC Spring 2023, 03/30/2023
//
...
module Operations =
  ...
  let average (R, G, B) = 
    let f = (((float R) * 0.299) + ((float G) * 0.587) + ((float B) * 0.114))
    let a = int(float f)
    (a, a, a)
  ...
  let rec Grayscale (width:int) 
    ...
    let a = List.map(fun x -> average x)
    let b = List.map(fun y -> a y) image 
    b
  ...
  let rec Threshold (width:int) 
                    ...
                    (image:(int*int*int) list list)
                    (threshold:int) = 
    let a = List.map(fun x -> t x threshold depth)
    let b = List.map(fun y -> a y) image 
    b
  ...
  let rec FlipHorizontal (width:int)
                         ...
                         (depth:int)
                         (image:(int*int*int) list list) = 
    let a = List.map(fun x -> List.rev x) image
    a
  ...
  let rec RotateRight90 (width:int)
                        ...
                        (depth:int)
                        (image:(int*int*int) list list) = 
    let a = List.transpose image
    List.map (fun x -> List.rev x) a
